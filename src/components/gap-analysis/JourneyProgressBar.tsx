...
interface JourneyProgressBarProps {
  ...
  onActionClick?: (target: 'avaliacao' | 'documentos' | 'remediacao') => void;
}

type JourneyAction = { label: string; target: 'avaliacao' | 'documentos' | 'remediacao' } | null;
...
function getContextualState(
  ...
  hasActionPlans: boolean,
  naoConformeCount: number
): { icon: typeof PlayCircle; iconColor: string; bgColor: string; message: string; action: JourneyAction } | null {
  ...
  if (evaluated === 0) {
    return {
      ...
      message: 'Comece avaliando os requisitos — clique em qualquer linha da tabela ou use a IA para analisar documentos.',
      action: { label: 'Analisar com IA', target: 'documentos' },
    };
  }
  if (pctConforme >= 80) {
    return {
  ...
  if (pctEvaluated >= 50 && naoConformeCount > 0 && !hasActionPlans) {
    return {
      ...
      action: { label: 'Ver remediação', target: 'remediacao' },
    };
  }
  if (pctEvaluated < 100) {
    return {
  ...
  return {
    ...
    action: naoConformeCount > 0 ? { label: 'Ver remediação', target: 'remediacao' } : null,
  };
}
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${state.bgColor}`}>
      ...
      {state.action && (
        <button
          type="button"
          onClick={() => onActionClick?.(state.action!.target)}
