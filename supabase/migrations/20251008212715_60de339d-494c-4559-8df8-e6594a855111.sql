-- Corrigir nome da tabela de sistemas privilegiados
...
BEGIN
  ...
  -- Sistemas Privilegiados (NOME CORRETO)
  ...
  INSERT INTO public.contas_privilegiadas (empresa_id, sistema_id, usuario_beneficiario, email_beneficiario, tipo_acesso, nivel_privilegio, data_concessao, data_expiracao, status, justificativa_negocio, aprovado_por, concedido_por, created_by)
  VALUES 
    (p_empresa_id, v_sistema_id, 'João Silva - CISO', 'joao.silva@empresa.com', 'Administrator', 'critico', CURRENT_DATE - 730, CURRENT_DATE + 365, 'ativa', 'Gestão infraestrutura crítica', p_user_id, p_user_id, p_user_id),
    (p_empresa_id, v_sistema_id, 'Carlos Santos - TI', 'carlos.santos@empresa.com', 'Root', 'critico', CURRENT_DATE - 600, CURRENT_DATE + 365, 'ativa', 'Administração servidores produção', p_user_id, p_user_id, p_user_id),
    (p_empresa_id, v_sistema_id, 'Carlos Santos - DBA', 'carlos.santos@empresa.com', 'Postgres Superuser', 'critico', CURRENT_DATE - 500, CURRENT_DATE + 365, 'ativa', 'Administração banco de dados', p_user_id, p_user_id, p_user_id),
    (p_empresa_id, v_sistema_id, 'Service Account Backup', 'backup@empresa.com', 'Service Account', 'alto', CURRENT_DATE - 400, CURRENT_DATE + 730, 'ativa', 'Serviço automatizado backup', p_user_id, p_user_id, p_user_id);
