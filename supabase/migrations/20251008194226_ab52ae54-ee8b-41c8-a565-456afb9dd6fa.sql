-- ================================
-- MIGRATION: Correção Final de Segurança - Última Função
-- Data: 2025-10-08
-- ================================

-- Corrigir as últimas funções que podem estar sem search_path

-- 1. update_updated_at_column (função genérica)
...
-- 2. calculate_due_diligence_score (função complexa)
        ...
        CASE 
          ...
          WHEN q.tipo = 'radio' THEN 
            CASE 
              ...
              ELSE 5
            END
          WHEN LENGTH(TRIM(r.resposta)) > 100 THEN 8
          WHEN LENGTH(TRIM(r.resposta)) > 50 THEN 6
          WHEN LENGTH(TRIM(r.resposta)) > 20 THEN 4
          ELSE 2
        END
      ) as pontuacao
